...
from .rgat_tuning import RGAT_Layer, Rel_Transformer_Layer
...
class T5LayerRTransformer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.rtransformer = Rel_Transformer_Layer(config.d_model, config.d_model, num_heads=1, feat_drop=0.2)
        self.filter = nn.Linear(config.d_ff, config.d_model, bias=False)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
    ...
    def map_cpu_to_gpu(self, graph_batch):
        for graph_dict in graph_batch:
            for key in graph_dict:
                graph_dict[key] = graph_dict[key].to('cuda')
        return graph_batch
    
    def graph_caption(self, hidden_states, graph_batch, relation_emb, **kwargs):
        '''
        :param hidden_states: [bsz x input_max_length x d_model]
        :return: hidden states with graph caption, while keeping the other reps
        '''
    ...
    def graph_caption_one(self, hidden_state, graph, edges, relation_emb, **kwargs):
